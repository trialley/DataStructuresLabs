	...
	*/
	void ReArrange() {
		for (int i = 0; i < _len; i++) {
			_LgSwap(_arr + i, _arr + LGRAND(0, _len - 1));
		}
		return;
	}
...
int main() {
	...
	cout << "及时终止选择排序测试---------" << endl;
	s.ReArrange();
	s.SelectSort();
	cout << "选择排序结束----------------" << endl << endl;

	cout << "及时终止冒泡排序测试---------" << endl;
	s.ReArrange();
	s.BubbleSort();
