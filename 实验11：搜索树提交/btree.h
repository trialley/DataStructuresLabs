	...
	void _cal (node* rootin, int* sizes, int* deepthes) {
		...
		if (rootin) {
			...
			if (rootin->left == nullptr && rootin->right == nullptr) {
				sizes[rootin->data.key] = 1;
				deepthes[rootin->data.key] = 1;
				//cout << deepthes[rootin->data.key] << " " << sizes[rootin->data.key] << "都无\n";

			//若当前元素有左子树，则以其为根的树的节点数与层数为左子树相应数据加1
			} else if (rootin->left != nullptr && rootin->right == nullptr) {
				sizes[rootin->data.key] = sizes[rootin->left->data.key] + 1;
				deepthes[rootin->data.key] = deepthes[rootin->left->data.key] + 1;
				//cout << deepthes[rootin->data.key] << " " << sizes[rootin->data.key] << "左有\n";

			//若当前元素有右子树，则以其为根的树的节点数与层数为右子树相应数据加1
			} else if (rootin->left == nullptr && rootin->right != nullptr) {
				sizes[rootin->data.key] = sizes[rootin->right->data.key] + 1;
				deepthes[rootin->data.key] = deepthes[rootin->right->data.key] + 1;
				//cout << deepthes[rootin->data.key] << " " << sizes[rootin->data.key] << "右都有\n";

			//若当前元素有左右子树，则以其为根的树的节点数与层数为左右子树相应数据相加再加1
			} else if (rootin->left != nullptr && rootin->right != nullptr) {
				sizes[rootin->data.key] = sizes[rootin->right->data.key] + sizes[rootin->left->data.key] + 1;
				deepthes[rootin->data.key] = max (deepthes[rootin->right->data.key], deepthes[rootin->left->data.key]) + 1;
				//cout << deepthes[rootin->data.key] <<" "<< sizes[rootin->data.key] << "左右都有\n";
			}
		}
		return;
	}
