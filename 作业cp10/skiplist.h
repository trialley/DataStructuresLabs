...
public:
	...
	void eraseMin () {
		//找到并保存最小值的指针
		skipNode<K,E>* to_delete = search ( headerNode->next[0]->element.first );
		//修正跳表结构
		for ( int i = 0; i < levels; i++ ) {
			if ( last[i]->next[i] != to_delete ) {
				break;
			}
			last[i]->next[i] = to_delete->next[i];
		}
		for ( ; levels > 0;levels-- ) {
			if ( headerNode->next[levels] != tailNode ) {
				break;
			}
		}
		//长度减一，删除元素
	...
	void eraseMax () {
		//找到并保存最大值的指针
		skipNode<K, E>* to_delete = search ( headerNode->next[levels]->element->first );
		//修正跳表结构
		for ( int i = 0; i < levels; i++ ) {
			if ( last[i]->next[i] != to_delete ) {
				break;
...
template<class K, class E>
skipList<K, E>::skipList ( K largeKey, int maxPairs, float prob ) {// Constructor for skip lists with keys smaller than largeKey and
...
template<class K, class E>
pair<const K, E>* skipList<K, E>::find ( const K& theKey ) const {// Return pointer to matching pair.
...
template<class K, class E>
int skipList<K, E>::level () const {// Return a random level number <= maxLevel.
	int lev = 0;
	while ( rand () <= cutOff )
		lev++;
...
template<class K, class E>
void skipList<K, E>::output ( ostream& out ) const {// Insert the dictionary pairs into the stream out.
