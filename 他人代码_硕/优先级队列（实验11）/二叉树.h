...
class linkedBinaryTree
{
	public:
		...
		void huffmanCode(char *ch)
		{//ch是节点的关键字，本算法的思想类似于层次遍历 
			arrayQueue< binaryTreeNode<T>* > q;
			arrayQueue< string > code;
			code.push("");
			binaryTreeNode<T> *t=root;
			while(t!=NULL)
			{
				if(t->left!=NULL){  q.push(t->left);  code.push(code.front()+"0"); } //由于是string类型可以用+连接 
				if(t->right!=NULL){  q.push(t->right);  code.push(code.front()+"1"); }
				if(t->left==NULL&&t->right==NULL)
				{//遇到了叶节点 
					cout<<ch[t->element]<<": "<<code.front()<<endl;
					//cout<<t->element<<code.front()<<endl;
				 }
