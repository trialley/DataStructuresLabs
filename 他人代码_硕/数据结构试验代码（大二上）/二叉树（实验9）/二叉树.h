...
class linkedBinaryTree
{
	public:
		...
		void makeTree(char *pre,char *in,int n)
		{
		...
		{//先判断两棵树根节点是否有空，若均不为空就比较其内的元素值 
		 //再判断左右子树    ------O(N)	 
			...
			{
				if(root1==NULL && root2==NULL) return true;   //根节点均为空，本条分支递归到终点 
		...
		void func(binaryTreeNode<T> *X)
		{
		    
			if(X->left==NULL) cout<<X->element<<"'s left is null"<<endl;
			else{
				cout<<X->element<<"'s left is not null"<<endl;
				func(X->left);
			}
				
			if(X->right==NULL) cout<<X->element<<"'s right is null"<<endl;
			else{
				cout<<X->element<<"'s right is not null"<<endl;
				func(X->right);
			}
...
/*void max_width(binaryTreeNode<T> *t)
		{//层次遍历二叉树*t，并计算哪一层的节点数最多 
		     arrayQueue< binaryTreeNode<T>* > q;  //这里先进先出所以用队列 
		     binaryTreeNode<T>* last=t;             //指向每层最后一个节点 
		     width[n+1]={0};  //每层节点数 
		     int i=1; width[1]=1;  //层号i 
		     while(t!=NULL)
		     {
			    visit(t);   //访问 t
			    if(t->left!=NULL)
			    {
			    	q.push(t->left);  width[i+1]++;
				}   
			    if(t->right!=NULL)
			    {
			    	q.push(t->right);  width[i+1]++;
				}  
				
				//访问下一个节点 
				if(q.empty()==true) break;  //已访问完所有元素 
			    t=q.front();
			    q.pop();
